$(document).ready(function(){
	...
	$('#log-dl').append('<p class="gg-login-lost-pw"><a href="####">忘记密码?</a></p>');
			...
			$.ajax({
				...
				success:function(data){
					...
					if(data.msgid == -4){
						...
						alert('登录成功');
						//eflower_alert.show('登录成功','乖乖学车',function(){
			...
			$.ajax({
				...
				success:function(data){
					...
					if(data.msgid == -5){
						...
						$('.reghintbox').eq(4).empty();
						alert('注册成功，点击登录');
						//eflower_alert.show('注册成功','乖乖学车',function(){
						//	localStorage.setItem('user_phone',tel)
						//	window.location.replace('../../html/loginReg_html/login_A.html');
						//})
					}
				}
			})
...
//	input 选中的border
	$(".inputbox").focus(function(e){
		//debugger;
		$('.inputbox').parent().css('border','1px solid gray');
